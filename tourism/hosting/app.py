...
with st.form("prediction_form"):
    ...
    if submitted:
        ...
        # 'Occupation': Freelancer -> 0, Large Business -> 1, Salaried -> 2, Small Business -> 3

        type_of_contact_mapping = {"Company Invited": 0, "Self Inquiry": 1}
        gender_mapping = {"Female": 0, "Male": 1, "Unknown": 2} # Include Unknown as per training
        ...
        # Assuming Xtrain columns are known or can be derived from the training script
        # For this example, let's assume the columns are in the order defined in the input_data DataFrame creation.
        # In a real scenario, you'd load Xtrain columns or ensure consistent feature engineering.
        # The columns below are taken from the original dataset description and the `data_preparation.py` script's `X` DataFrame.
        expected_columns = [
        ...
        # This assumes that `model` was trained on features in this exact order
